    ...
    <div class="bg-white border-b border-gray-100">
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div>
                ...
                <p class="text-body text-gray-500 mt-0.5">Rekap data operasional pengiriman</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <div class="bg-white rounded-[10px] border border-gray-100 p-4 text-center">
                ...
                <p class="text-caption text-gray-500 mt-0.5">Total Box</p>
            </div>
            <div class="bg-white rounded-[10px] border border-gray-100 p-4 text-center">
                <p class="text-[22px] font-bold text-gray-900">{{ number_format($totalItems) }}</p>
                <p class="text-caption text-gray-500 mt-0.5">Total Barang</p>
            </div>
            <div class="bg-white rounded-[10px] border border-gray-100 p-4 text-center">
        ...
        <div class="bg-white rounded-[12px] border border-gray-100 p-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <div class="lg:col-span-2 relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari tracking, batch, atau customer..." class="w-full pl-10 pr-4 py-2.5 text-body bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-2 focus:ring-accent/40 transition-colors placeholder:text-gray-400">
                </div>
                ...
                <select wire:model.live="filterMethod" class="py-2.5 px-3 text-body bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-2 focus:ring-accent/40 transition-colors text-gray-600">
                    <option value="">Semua Metode</option>
                    <option value="air">Air</option>
                    <option value="sea">Sea</option>
                </select>
                <div class="flex items-center gap-2">
                    <input type="date" wire:model.live="filterDateFrom" class="w-full px-3 py-2.5 text-body bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-2 focus:ring-accent/40 transition-colors text-gray-600" title="Dari Tanggal">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mt-3">
                <div class="lg:col-span-2"></div>
                <div class="lg:col-span-2 flex items-center gap-2">
                    <span class="text-caption text-gray-500 whitespace-nowrap">Sampai:</span>
                    <input type="date" wire:model.live="filterDateTo" class="w-full px-3 py-2.5 text-body bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-2 focus:ring-accent/40 transition-colors text-gray-600">
                </div>
                <div class="flex items-center">
                    <button wire:click="$set('search', ''); $set('filterType', ''); $set('filterMethod', ''); $set('filterDateFrom', ''); $set('filterDateTo', ''); loadStats()" class="px-3 py-2.5 text-caption font-medium text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 rounded-[8px] transition-colors w-full text-center">
                        Reset Filter
                    </button>
                </div>
            </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-100">
                                ...
                                <th class="text-center px-5 py-3 text-caption font-semibold text-gray-500 uppercase tracking-wide">Barang</th>
                                ...
                                <th class="text-left px-5 py-3 text-caption font-semibold text-gray-500 uppercase tracking-wide">Dibuat</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-5 py-3.5">
                                        <div>
                                            ...
                                            @if($box->notes)
                                                <p class="text-caption text-gray-400 truncate max-w-[200px]">{{ $box->notes }}</p>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-5 py-3.5">
                                    ...
                                    <td class="px-5 py-3.5">
                                        <span class="inline-flex items-center gap-1 text-caption font-medium uppercase {{ $box->method === 'air' ? 'text-blue-600' : 'text-cyan-600' }}">
                                            <span class="w-1.5 h-1.5 rounded-full {{ $box->method === 'air' ? 'bg-blue-400' : 'bg-cyan-400' }}"></span>
                                            {{ $box->method }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-3.5 text-center">
                                        <span class="text-body text-gray-700 font-medium">{{ $box->items_count }}</span>
                                    </td>
                                    <td class="px-5 py-3.5">
                                        <x-status-badge :status="$box->status" />
                                    </td>
                                    <td class="px-5 py-3.5">
                                        <span class="text-caption {{ $box->etd ? 'text-gray-700' : 'text-gray-400' }}">{{ $box->etd ? $box->etd->format('d/m/y') : '-' }}</span>
                                    </td>
                                    <td class="px-5 py-3.5">
                                        <span class="text-caption {{ $box->eta ? 'text-gray-700' : 'text-gray-400' }}">{{ $box->eta ? $box->eta->format('d/m/y') : '-' }}</span>
                                    </td>
                                    <td class="px-5 py-3.5">
                                        <span class="text-caption text-gray-500">{{ $box->created_at->format('d M Y') }}</span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="md:hidden space-y-3">
                @foreach($boxes as $box)
                    <div class="bg-white rounded-[12px] border border-gray-100 p-4">
                        ...
                        <div class="grid grid-cols-3 gap-2 text-caption py-3 border-t border-gray-100">
                            <div>
                        ...
                        <div class="flex items-center gap-4 text-caption text-gray-500 pt-2 border-t border-gray-100">
