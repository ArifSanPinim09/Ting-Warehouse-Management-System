    ...
    <div class="bg-white border-b border-gray-100">
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div>
                ...
                <p class="text-[13px] text-gray-500 mt-0.5">Perbarui estimasi pengiriman (ETD/ETA) untuk box aktif</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
        ...
        <div class="bg-white rounded-[12px] border border-gray-100 p-4">
            <div class="relative max-w-lg">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari tracking number, batch, atau customer..." class="w-full pl-10 pr-4 py-2.5 text-[13px] bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-1 focus:ring-accent/20 transition-colors placeholder:text-gray-400">
            </div>
        </div>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($boxes as $box)
                                <tr class="hover:bg-gray-50/50 transition-colors {{ $selectedBoxId === $box->id ? 'bg-blue-50/30' : '' }}">
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-[11px] text-gray-400 capitalize">{{ $box->type }} · {{ strtoupper($box->method) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-3.5">
                                    ...
                                    <td class="px-5 py-3.5">
                                        @if($box->etd)
                                            <span class="text-[13px] text-gray-700">{{ $box->etd->format('d M Y') }}</span>
                                        @else
                                            <span class="text-[12px] text-gray-400 italic">Belum diset</span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-3.5">
                                        @if($box->eta)
                                            <span class="text-[13px] text-gray-700">{{ $box->eta->format('d M Y') }}</span>
                                        @else
                                            <span class="text-[12px] text-gray-400 italic">Belum diset</span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-3.5 text-right">
                                        <button wire:click="selectBox({{ $box->id }})" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium text-accent hover:text-primary bg-accent/5 hover:bg-accent/10 rounded-[6px] transition-colors">
            ...
            <div class="md:hidden space-y-3">
                @foreach($boxes as $box)
                    <div class="bg-white rounded-[12px] border border-gray-100 p-4 {{ $selectedBoxId === $box->id ? 'border-blue-300 bg-blue-50/20' : '' }}">
                        ...
                        <div class="grid grid-cols-2 gap-2 text-[12px] py-3 border-t border-gray-100">
                            <div>
                        ...
                        <button wire:click="selectBox({{ $box->id }})" class="w-full mt-2 flex items-center justify-center gap-1.5 px-3 py-2 text-[12px] font-medium text-accent hover:text-primary bg-accent/5 hover:bg-accent/10 rounded-[8px] transition-colors">
                            Update Estimasi
                        </button>
                    </div>
                @endforeach
            ...
            <div class="flex min-h-full items-end sm:items-center justify-center p-4">
                <div class="relative bg-white rounded-[16px] shadow-modal w-full max-w-md p-6 transform transition-all" @click.stop>
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <button wire:click="cancelForm" class="p-1 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-[12px] font-medium text-gray-700 mb-1.5">ETD (Estimated Time Departure)</label>
                            <input type="date" wire:model="etd" class="w-full px-3 py-2.5 text-[13px] bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-1 focus:ring-accent/20 transition-colors">
                            @error('etd') <p class="text-[12px] text-red-500 mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-[12px] font-medium text-gray-700 mb-1.5">ETA (Estimated Time Arrival)</label>
                            <input type="date" wire:model="eta" class="w-full px-3 py-2.5 text-[13px] bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-1 focus:ring-accent/20 transition-colors">
                            @error('eta') <p class="text-[12px] text-red-500 mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-[12px] font-medium text-gray-700 mb-1.5">Catatan (opsional)</label>
                            <textarea wire:model="estNote" rows="2" placeholder="Tambahkan catatan perubahan..." class="w-full px-3 py-2.5 text-[13px] bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-1 focus:ring-accent/20 transition-colors resize-none"></textarea>
                        </div>
                    </div>

                    <div class="flex items-center gap-3 justify-end mt-6">
                        <button wire:click="cancelForm" class="px-4 py-2.5 text-[13px] font-medium text-gray-700 bg-white border border-gray-200 rounded-[8px] hover:bg-gray-50 transition-colors">
                            Batal
                        </button>
                        <button wire:click="saveEstimate" class="px-4 py-2.5 text-[13px] font-medium text-white bg-primary rounded-[8px] hover:bg-primary-light transition-colors">
                            Simpan Estimasi
                        </button>
                    </div>
                </div>
