    ...
    <div class="bg-white border-b border-gray-100">
        <div class="mx-auto w-full max-w-[1500px] px-4 md:px-6 lg:px-8 xl:px-10 2xl:px-12 py-6">
            <div>
                ...
                <p class="text-body text-gray-500 mt-0.5">Verifikasi bukti transfer dari customer</p>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-[12px] border border-gray-100 p-4">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="sm:col-span-2 relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari invoice number atau customer..." class="w-full pl-10 pr-4 py-2.5 text-body bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-2 focus:ring-accent/40 transition-colors placeholder:text-gray-400">
                </div>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @foreach($invoices as $inv)
                                        <tr class="hover:bg-gray-50/50 transition-colors cursor-pointer {{ $inv->status === 'waiting_verification' ? 'bg-amber-50/30' : '' }}" wire:click="selectInvoice({{ $inv->id }})">
                                            ...
                                            <td class="px-5 py-3.5">
                                                <span class="text-caption font-medium text-gray-600 capitalize">{{ $inv->payment_method ?? '-' }}</span>
                                            </td>
                                            ...
                                            <td class="px-5 py-3.5">
                                                <span class="text-caption text-gray-500">{{ $inv->updated_at->format('d M Y H:i') }}</span>
                                            </td>
                                            <td class="px-5 py-3.5 text-right">
                        ...
                        @foreach($invoices as $inv)
                            <div wire:click="selectInvoice({{ $inv->id }})" class="bg-white rounded-[12px] border border-gray-100 p-4 hover:shadow-card-hover transition-shadow cursor-pointer {{ $inv->status === 'waiting_verification' ? 'border-l-4 border-l-amber-400' : '' }}">
                                ...
                                <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                                    <span class="text-caption text-gray-500 capitalize">{{ $inv->payment_method ?? '-' }}</span>
                ...
                <div class="w-full lg:w-[420px] flex-shrink-0">
                    <div class="bg-white rounded-[12px] border border-gray-100 sticky top-[88px]">
                        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                            <h3 class="text-body font-semibold text-gray-900">Detail Pembayaran</h3>
                        ...
                        <div class="p-5 space-y-5">
                            ...
                            <div class="text-center pb-4 border-b border-gray-100">
                                <p class="text-caption text-gray-400 uppercase tracking-wide mb-1">Invoice</p>
                                <p class="text-[18px] font-bold text-gray-900">{{ $selectedInvoice->invoice_number }}</p>
                                <p class="text-body text-gray-500 mt-1">{{ $selectedInvoice->customer->name ?? '-' }}</p>
                            </div>

                            {{-- Amount --}}
                            <div class="text-center py-4 bg-gray-50 rounded-[10px]">
                                <p class="text-caption text-gray-400 uppercase tracking-wide mb-1">Total Tagihan</p>
                            ...
                            @if($selectedInvoice->payment_proof)
                                <div class="border-t border-gray-100 pt-4">
                                    <p class="text-caption font-semibold text-gray-500 uppercase tracking-wide mb-3">Bukti Transfer</p>
                                    ...
                                    <p class="text-caption text-gray-400 mt-2 text-center">Klik gambar untuk memperbesar</p>
                                </div>
                            @else
                                <div class="border-t border-gray-100 pt-4">
                                    <div class="flex flex-col items-center py-6 text-center">
                                        <svg class="w-10 h-10 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        <p class="text-body text-gray-400">Belum ada bukti transfer</p>
                                    </div>
                                </div>
                            @endif
                ...
                <div class="relative bg-white rounded-[16px] shadow-modal w-full max-w-md p-6 transform transition-all" @click.stop>
                    <div class="flex items-start gap-4 mb-5">
                        ...
                        <div>
                            ...
                            <p class="text-body text-gray-500 mt-1">Pembayaran akan ditolak dan customer akan diminta mengupload ulang. Berikan alasan penolakan.</p>
                        </div>
                    </div>

                    <div class="mb-5">
                        <label class="block text-caption font-medium text-gray-700 mb-1.5">Alasan Penolakan <span class="text-red-500">*</span></label>
                        <textarea
                            wire:model="rejectReason"
                            rows="3"
                            placeholder="Jelaskan alasan penolakan..."
                            class="w-full px-3 py-2.5 text-body bg-white border border-gray-200 rounded-[8px] focus:border-accent focus:ring-2 focus:ring-accent/40 transition-colors resize-none"
                        ></textarea>
                        @error('rejectReason') <p class="text-caption text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex items-center gap-3 justify-end">
                        <button wire:click="$set('showRejectModal', false)" class="px-4 py-2.5 text-body font-medium text-gray-700 bg-white border border-gray-200 rounded-[8px] hover:bg-gray-50 transition-colors">
                            Batal
                        </button>
                        <button
                            wire:click="rejectPayment"
                            class="px-4 py-2.5 text-body font-medium text-white bg-red-600 rounded-[8px] hover:bg-red-700 transition-colors"
                        >
                            Tolak Pembayaran
                        </button>
                    </div>
