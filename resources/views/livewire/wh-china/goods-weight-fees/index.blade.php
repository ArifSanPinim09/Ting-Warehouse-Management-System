<div class="min-h-screen bg-[#f8fafc]">
    ...
    <div class="bg-white border-b border-gray-100">
        <div class="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <h1 class="text-[20px] font-bold text-primary">Goods Weight Fee</h1>
            <p class="text-[13px] text-gray-500 mt-1">Input goods weight fees in Yuan — Owner views in Rupiah</p>
        </div>
    </div>
    ...
    <div class="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {{-- Kurs Info --}}
        <div class="bg-blue-50 border border-blue-100 rounded-[8px] p-4 flex items-center justify-between">
            <div>
                <p class="text-[12px] text-blue-600 font-medium">Current Kurs (Yuan → IDR)</p>
                <p class="text-[20px] font-bold text-blue-900">¥1 = Rp {{ number_format($kurs, 0, ',', '.') }}</p>
            </div>
        </div>

        {{-- Form --}}
        <div class="bg-white rounded-[12px] border border-gray-100 p-6">
            <h2 class="text-[15px] font-semibold text-gray-900 mb-4">{{ $editId ? 'Edit Fee' : 'Add Goods Weight Fee' }}</h2>
            <form wire:submit="save" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-[12px] font-medium text-gray-600 mb-1">Box</label>
                    <select wire:model.live="boxId" class="w-full border border-gray-200 rounded-[8px] px-3 py-2 text-[13px] focus:border-primary outline-none">
                        <option value="">Pilih box...</option>
                        @foreach($boxes as $box)
                            <option value="{{ $box->id }}">{{ $box->display_name }} ({{ strtoupper($box->method) }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-[12px] font-medium text-gray-600 mb-1">Huruf Box</label>
                    <input wire:model="hurufBox" type="text" class="w-full border border-gray-200 rounded-[8px] px-3 py-2 text-[13px] focus:border-primary outline-none" readonly>
                </div>
                <div>
                    <label class="block text-[12px] font-medium text-gray-600 mb-1">Berat (kg)</label>
                    <input wire:model="beratKg" type="number" step="0.01" class="w-full border border-gray-200 rounded-[8px] px-3 py-2 text-[13px] focus:border-primary outline-none">
                    @error('beratKg') <p class="text-[12px] text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
        ...
        <div class="bg-white rounded-[12px] border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <div class="text-right text-[12px]">
                    <p class="text-gray-400">Total Berat: <span class="font-semibold text-gray-900">{{ number_format($totalBerat, 2) }} kg</span></p>
                    <p class="text-gray-400">Total: <span class="font-semibold text-gray-900">¥{{ number_format($totalYuan, 2) }}</span> ≈ Rp {{ number_format($totalRupiah, 0, ',', '.') }}</p>
                </div>
            </div>

            @if($fees->isEmpty())
                <div class="p-12 text-center text-[13px] text-gray-400">No goods weight fees recorded yet.</div>
            @else
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-gray-50/80 border-b border-gray-100">
                                <th class="px-4 py-3 text-[12px] font-semibold text-gray-600 uppercase">Box</th>
                                <th class="px-4 py-3 text-[12px] font-semibold text-gray-600 uppercase">Huruf</th>
                                <th class="px-4 py-3 text-[12px] font-semibold text-gray-600 uppercase">Berat (kg)</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($fees as $fee)
                                <tr class="hover:bg-gray-50/50">
                                    <td class="px-4 py-3 text-[13px] font-medium text-gray-900">{{ $fee->box?->display_name ?? '-' }}</td>
                                    <td class="px-4 py-3 text-[13px] text-gray-600">{{ $fee->huruf_box ?? '-' }}</td>
                                    <td class="px-4 py-3 text-[13px] text-gray-600">{{ number_format($fee->berat_kg, 2) }}</td>
