...
@php
    $icons = [
        ...
        'calculator' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>',
...
<div class="ds-empty">
    <svg class="ds-empty-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        {!! $icons[$icon] ?? $icons['box'] !!}
    </svg>
    <h3 class="ds-empty-title">{{ $title }}</h3>
    <p class="ds-empty-text">{{ $text }}</p>
    @if($action)
        <a href="{{ $actionUrl }}" wire:navigate class="ds-btn-primary">
            {{ $action }}
        </a>
    @endif
    {{ $slot }}
